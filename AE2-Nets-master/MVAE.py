...
"""
#!联合更新的MVAE只有这一个网络，没有前面的AE结构,更改自ae_binae_joint
"""
...
class MaeAEModel:
    ...
    def train_model(self,X1, X2, gt, epochs, batch_size):
        ...
        encoded,z_mean,z_log_var=self.maeencoder([x1_input0,x2_input0])
        ...
        lamb=self.lamb
        maeloss=lamb*K.sum(1*K.mean((x1_input0-decoder_output[0])**2,0))+lamb*K.sum(1*K.mean((x2_input0-decoder_output[1])**2,0))+vae_mse_loss
    ...
    def encoder(self,x1,dims): #dims=[input_shape,200]
        ...
        """
    def decoder(self,dims): #dims=[200,input_shape]
        h=z=Input(shape=(dims[0],))
        for i in range(len(dims)-1): 
            h=Dense(dims[i+1],activation="relu")(h)
    ...
    def mae_encoder(self,  dims1,dims2,h_dim=64):   #dims1=[200,100,32],dims2=[200,100,32],一般地，dims1[-1]+dims2[-1]=h_dim
        h1=input1=Input(shape=(dims1[0],))
        h2=input2=Input(shape=(dims2[0],))
        for i in range(len(dims1)-1):
            h1=Dense(dims1[i+1],activation='tanh',kernel_regularizer=regularizers.l2(self.reg_lambda))(h1)
