...
class RBM(object):
    ...
    def rbm_output_reverse(self,X):
        ...
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            return sess.run(out)
def create_RBMlist(input_size,RBM_hidden_sizes):
    rbm_list=[]
...
def model(X1, X2, gt, para_lambda, dims, act, lr, epochs, batch_size):
    ...
    #创建RBM列表
    RBM_hidden_sizes = [500, 200 ] 
    rbm0_list=create_RBMlist(240,RBM_hidden_sizes)
    rbm1_list=create_RBMlist(216,RBM_hidden_sizes)
