...
if __name__ == '__main__':
    ...
    epochs_h = 20
    act = 'sigmoid'
    dims_ae = [[X[str(0)].shape[1], 256], [X[str(1)].shape[1], 2048, 256], [X[str(2)].shape[1], 2048, 256]]
    dims_dg = [[256, 256], [256, 256], [256, 256]]
    ...
    for j in range(num):
        ...
        H, gt = model_multi_view(X, gt, para_lambda, dims_ae, dims_dg, act, lr, epochs, batch_size, False)
    ...
    arg = ['lambda', para_lambda, 'batch_size', batch_size, 'lr_pre', lr_pre, 'lr_ae', lr_ae,
           'lr_dg', lr_dg, 'lr_h', lr_h, 'epoch_pre', epochs_pre, 'epochs_total', epochs_total, 'dims_ae', dims_ae,
           'dims_dg', dims_dg]
    files = open('./result/result_COIL20_3views.csv', 'a')
